...
const projects: Project[] = [
  { id: 'autismart', name: 'AUTISMART', type: 'AI / HEALTHCARE', detail: 'Multimodal ASD detection, adaptive therapy, and progress reporting.', result: 'FYP-I A+ · FYP-II A- · IEEE-format paper', link: 'https://alishah1029384756.github.io/AliShah1029384756/projects/autismart.html', position: [-2.25, .8, 0], color: '#00f5ff' },
  { id: 'clinicos', name: 'CLINICOS', type: 'OPERATIONS', detail: 'Scheduling-to-billing workflow for modern clinics.', result: 'Manual operations shifted to systemized execution.', link: 'https://alishah1029384756.github.io/AliShah1029384756/projects/clinicos.html', position: [2.25, .65, .2], color: '#ff2bd6' },
  { id: 'schooliep', name: 'SCHOOLIEP', type: 'EDUCATION', detail: 'Structured IEP records with role-based access.', result: 'Auditable record lifecycle for school requirements.', link: 'https://alishah1029384756.github.io/AliShah1029384756/projects/schooliep.html', position: [-1.55, -1.45, -.25], color: '#b8ff38' },
  { id: 'educonnect', name: 'EDUCONNECT', type: 'COMMUNITY', detail: 'Student support, resources, forums, and analytics.', result: 'One access point for academic and community support.', link: 'https://github.com/AliShah1029384756/EduConnect', position: [1.65, -1.4, -.1], color: '#9b6cff' },
]
...
function Hologram({ reduced }: { reduced: boolean }) {
  ...
  return <group ref={ref}>
    ...
    {[-.28, .28].map((y) => <mesh key={y} position={[0, y, .33]} scale={[.55, .025, .025]}><boxGeometry args={[1, 1, 1]} /><meshBasicMaterial color="#00f5ff" /></mesh>)}
    ...
    <Text position={[0, -1.3, 0]} fontSize={.1} color="#00f5ff" anchorX="center" letterSpacing={.1}>ALI / HOLOGRAM CORE</Text>
  </group>
}
...
export function SpaceProfile() {
  ...
  return <section className={`space-profile cyber-profile ${entered ? 'is-entered' : 'is-landing'}`} aria-label="Immersive engineering portfolio">
    <div className="space-topbar cyber-topbar"><a href="#top" className="space-brand"><span>SA</span><b>SYED MUHAMMAD ALI NAQVI / SYS-01</b></a><span className="space-status"><i /> NEURAL LINK ONLINE · 2026</span></div>
    {!entered && <div className="space-landing cyber-landing"><p className="space-kicker">FULL-STACK ENGINEER · FAST-NUCES GRADUATE 2026</p><h1>ENTER THE<br /><em>NEURAL GRID</em></h1><p>Engineering intelligent systems for healthcare, education, and the people who use them.</p><div className="hero-actions"><button className="enter-button" type="button" onClick={() => setEntered(true)}>INITIALIZE PROFILE <span>↗</span></button><a className="skip-link" href="#about">SKIP TO PORTFOLIO</a></div><small>Drag to orbit · Select a node · Sound muted by default</small></div>}
    ...
    {entered && <><div className="space-hud cyber-hud"><span className="hud-label">ACTIVE NODE / {current.type}</span><strong>{current.name}</strong><small>{current.result}</small><div className="hud-rule" /><span>{current.detail}</span><a href={current.link} target="_blank" rel="noopener noreferrer">OPEN CASE STUDY ↗</a></div><nav className="space-spaces cyber-spaces" aria-label="Portfolio sections">{sections.map(([label, detail, href]) => <a key={label} href={href}>{label}<small>{detail}</small></a>)}</nav><form className="space-command cyber-command" onSubmit={(event) => { event.preventDefault(); runCommand(command) }}><span>›</span><input aria-label="Navigate portfolio" value={command} onChange={(event) => setCommand(event.target.value)} placeholder="type: projects / autismart / contact" /></form><div className="cyber-actions"><button type="button" onClick={toggleSound} aria-pressed={sound}>{sound ? 'SOUND ON' : 'SOUND MUTED'}</button><button type="button" onClick={() => setEntered(false)}>EXIT GRID</button></div></>}
  </section>
}
